...
sat_years = list(set([int(x) if not math.isnan(x) else 0 for x in sat_data['Day']]))
...
for index, row in ufo_sighting.iterrows():
	...
	try:
		sighted_at_year = row['sighted_at']
	except:
	...
	if int(sighted_at_year) in sat_years:
		df_sat_by_year = sat_data.loc[sat_data['Day'] == sighted_at_year]
...
ufo_sighting.to_csv('reference_w_satellite_day.csv', index=False)
